      ...
      <div class="table">
        ...
        <el-table v-loading="loading" :data="tableData" @selection-change="handleSelectionChange" stripe
          ...
          <el-table-column label="操作" width="480" fixed="right">
            <template slot-scope="scope">
              <el-button   type="info" v-if="scope.row.state == 0"
                @click="toEdit(scope.row.id, 1, scope.row.attractionsId)">上架</el-button>
              <el-button   type="info" v-if="scope.row.state == 1"
                @click="toEdit(scope.row.id, 0, scope.row.attractionsId)">下架</el-button>
              <el-button   type="primary" @click="toItem(scope.row.id)">房型</el-button>
