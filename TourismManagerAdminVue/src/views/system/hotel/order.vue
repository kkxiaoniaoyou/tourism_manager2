      ...
      <div class="table">
        ...
        <el-table v-loading="loading" :data="tableData" @selection-change="handleSelectionChange" stripe
          ...
          <el-table-column label="操作" width="540" fixed="right">
            <template slot-scope="scope">
              <el-button   type="warning" @click="editSysHotelOrder(scope.row.id, 5)">已使用</el-button>
              <el-button   v-if="scope.row.state == 2" type="primary"
                @click="editSysHotelOrder(scope.row.id, 4)">不可取消</el-button>
              <el-button   v-if="scope.row.state == 2" type="primary"
                @click="editSysHotelOrder(scope.row.id, 3)">取消</el-button>
              <el-button   v-if="scope.row.state == 0" type="primary"
                @click="editSysHotelOrder(scope.row.id, 1)">确认</el-button>
