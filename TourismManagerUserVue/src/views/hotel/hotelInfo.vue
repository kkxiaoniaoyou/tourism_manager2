<template>
  <div class="attractionsInfo">
    ...
    <el-dialog title="预定" :visible.sync="dialogVisible" width="40%">
      <span>
        <div>
          <el-radio style="margin-top:10px" v-for="(item, index) in hotel" :key="index" v-model="radio1" :label="item.id"
            border>{{ item.name }} - 数量{{ item.num }} - 价格{{ item.price }}</el-radio>
        </div>
        <el-input-number style="margin-top:10px" v-model="num" :min="1" :max="10"></el-input-number>
        <el-date-picker style="margin-left:20px;margin-top:10px" v-model="date1" type="date" value-format="yyyy-MM-dd"
          placeholder="选择预定日期">
        </el-date-picker>
