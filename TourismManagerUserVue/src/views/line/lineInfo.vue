    ...
    <div class="forumInfo1">
        <div class="forumInfo2">
            <div class="forumInfo3">
                {{info.name}}
                <el-button v-if="!flag"  type="success" icon="iconfont icon-r-add" circle @click="saveSysFavor"></el-button>
                <el-button v-if="flag"  type="warning" icon="iconfont icon-r-delete" circle @click="removeSysFavor"></el-button>
            </div>
        ...
        saveSysFavor(param).then(res => {
          if (res.code == 1000) {
            this.$message({
                message: '收藏成功',
                type: 'success',
                duration: 1000  // 设置为 1.5 秒（1500 毫秒）
            });
            this.getSysFavor()
          }
        })
